...
export async function analyzeReadme({ content }: AnalyzeInput): Promise<AnalyzeResult> {
  ...
  if (!apiKey) {
    return { success: false, error: "AI is not configured for this project (Missing GOOGLE_GENERATIVE_AI_API_KEY in .env)." };
  }
  ...
  try {
    ...
    return { success: true, pitch: normalize(result.object) };
  } catch (error) {
